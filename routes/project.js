...
function addTopics(req, res) {
  ...
  if (!isValid)
    return res.status(400).json(Response.error("Invalid request!", errors));
  ...
  readOne(STUDENT, { matricNo })
    .then((student) => {
      if (student === null)
        return res
          .status(400)
          .json(Response.error("No such student with that matric number!"));
      ...
      readOne(PROJECT, { owner: student.id })
        ...
        .then(async (project) => {
          ...
          if (project.approvedTopic)
            return res
              .status(400)
              .json(Response.error("A topic has already been approved!"));

          const associatedTopics = await readMany(TOPIC, {
          ...
          if (associatedTopics.length >= 3)
            return res
              .status(400)
              .json(Response.error("3 Topics have already been proposed!"));
          ...
          const alreadyPoposedTopics = associatedTopics.filter((topic) =>
            topics.includes(topic.title)
          );
    ...
    .catch((err) => {
      logger.error(
        `Failed to readStudent with matricNo: ${matricNo} - failed with message - ${err.message}`
      );
      res.status(500).json(Response.fatal());
    });
}
...
async function approveProject(req, res) {
  ...
  exists(LECTURER, { _id: lecturerId })
    .then((lecturerExists) => {
      if (!lecturerExists)
        return res.status(400).json(Response.error("Unknown LecturerId!"));
    ...
    .then(async (topic) => {
      if (!topic)
        return res
          .status(400)
          .json(Response.error("Could not find that topic to approve!"));
      ...
      try {
        ...
        if (project.approvedTopic)
          return res
            ...
            .json(
              Response.error(
                "A topic was previously approved for this project!"
              )
            );
      } catch (error) {
        logger.error(
          `Failed to read project - failed with message - ${error.message}`
        );
      ...
      await updateMany(
        TOPIC,
        { projectId: topic.projectId, reviewed: false },
        { reviewed: true }
      );
      ...
      update(
        PROJECT,
        { _id: topic.projectId },
        { approvedTopic: topic.id, approvedBy: lecturerId }
      )
        .then(() => {
        ...
        .catch((err) => {
          logger.error(
            `Failed to approve topic - failed with message - ${err.message}`
          );
          res.status(500).json(Response.fatal());
        });
    })
  ...
  readOne(PROJECT, { _id: projectId })
    .then((project) => {
      if (!project)
        return res
          .status(400)
          .json(Response.error("Could not find that project to review!"));
      ...
      if (project.approvedTopic)
        return res
          ...
          .json(
            Response.error(
              "Cannot reject topics on a project that has an approved topic!"
            )
          );

      updateMany(
        TOPIC,
        { projectId: project.id, reviewed: false },
        { reviewed: true }
      )
        .then(() => {
        ...
        .catch((err) => {
          logger.error(
            `Failed to review project - failed with message - ${err.message}`
          );
          res.status(500).json(Response.fatal());
        });
    })
  ...
  readMany(PROJECT, { approvedTopic: null }, { populate: ["owner"] })
    ...
    .then((data) => {
      ...
      const projectMap = projects.reduce(
        (acc, proj) => ({ ...acc, [proj.id]: { ...proj, topics: [] } }),
        {}
      );
      ...
      const grouped = topics.reduce((groupedSoFar, topic) => {
        ...
        if (groupedSoFar[associatedProject])
          groupedSoFar[associatedProject].topics = [
            ...groupedSoFar[associatedProject].topics,
            topic,
          ];
        return groupedSoFar;
      ...
      const filteredGrouped = Object.values(grouped).filter(
        (project) => project.topics.length > 0
      );
      res.status(200).json(Response.success("Done!", filteredGrouped));
    })
    .catch((err) => {
      logger.error(
        `Failed to read unapproved topics - failed with message - ${err.message}`
      );
      res.status(500).json(Response.fatal());
    });
}
  ...
  readOne(STUDENT, { matricNo })
    .then((student) => {
      ...
      return readOne(
        PROJECT,
        { owner: student.id },
        { populate: ["approvedBy", "approvedTopic"] }
      );
    })
    .then((project) => {
    ...
    .then((project) => {
      ...
      if (
        ...
        save(UPLOAD, uploadDetails)
          .then((savedDoc) => {
            res.status(200).json(Response.success("Done.", uploadDetails));
          })
          .catch((err) => {
            logger.error(
