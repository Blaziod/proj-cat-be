...
// updates existing uploads or create new ones if none was previously created
async function uploadDoc(req, res) {
	...
	// read a project for this student
	readOne(TOPIC, { _id: uploadDetails.topic })
		.then(async topic => {
			if (topic === null) {
			...
			if (topic === null || !topic.reviewed) {
				// no approved topic, reject
				res.status(400).json(Response.error('Topic not you reviewed'))
			} else {
				// check if a document has already been uploaded and replace it instead
				return await readOne(UPLOAD, { topic: uploadDetails.topic })
			}
		})
		.then(async existingUpload => {
			// overwrite if existing
			if (existingUpload) return await update(UPLOAD, { _id: existingUpload._id }, uploadDetails)
			// save new if not
			return await save(UPLOAD, uploadDetails)
		})
		.then(savedUpload => {
			res.status(200).json(Response.success('Done.', savedUpload))
		})
		.catch(err => {
			logger.error(`failed to save upload - failed with message - ${err.message}.\n`)
			res.status(500).json(Response.fatal())
		})
}

// get uploaded docs
function getUploads(_, res) {
	readMany(UPLOAD, {}, { populate: ['topic', 'student'] })
		.then(uploads => {
			res.status(200).json(Response.success('Done.', uploads))
		})
		.catch(err => {
			logger.error(`failed to read uploads - failed with message - ${err.message}.\n`)
			res.status(500).json(Response.fatal())
		})
}
